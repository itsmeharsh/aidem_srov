...
//                if ((isset($val)) && $val == "eStatus")
//                {
//                    $pageName = $this->ci->uri->segment(2); 
//                    if($pageName=='request')
//                    {
//                       $eStatusPossibleValues = enum_possible_values(TBL_PARTNER, 'eStatus');                     
//                        $temp_eStatusPossibleValues = $eStatusPossibleValues;
//                        unset($temp_eStatusPossibleValues[$replace_string]);
//                        if (count($temp_eStatusPossibleValues) > 0) 
//                        {                         
//                            $STATUS_BTN = '<div class="input-group-btn"><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown">';      
//                            $STATUS_BTN.= $replace_string;
//                            $STATUS_BTN.= '<span class="caret"></span>';
//                            $STATUS_BTN.= '</button>';
//                            $STATUS_BTN.= '<ul class="dropdown-menu">';                                                      
//                            foreach ($temp_eStatusPossibleValues as $possibleKey => $possibleValue) 
//                            {                             
//                               $STATUS_BTN .= '<li>' . anchor('javascript:;', $possibleValue,array('id' => end($row_data),'class'=>'btn-click','status'=>$possibleValue)) . '</li>';
//                            }
//                            $STATUS_BTN .= '</ul>';
//                            unset($temp_eStatusPossibleValues);
//                            $STATUS_BTN .= '</div>';                                                  
//                        }                      
//                    $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), $STATUS_BTN, str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));  
//                    }
//                    else if($pageName=='editor')
//                    {                      
//                        $eStatusPossibleValues = array("Add as Editor","Remove as Editor");
//                       
//                        $STATUS_BTN = array(
//                                                'content' => ($replace_string == 'Add as Editor')? 'Remove From Editor' : 'Add as Editor',
//                                                'class' => 'btn btn-click btn-sm'.(($replace_string != 'Add as Editor') ? ' btn-success' : ' btn-warning'),
//                                                'id' => end($row_data)
//                                           );
//                        $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), anchor('javascript:;', form_button($STATUS_BTN)), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));  
//                    }
//                    else   
//                    {
//                      $eStatusPossibleValues = enum_possible_values(TBL_USER, 'eStatus');  
//                        $STATUS_BTN = array(
//                            'content' => $replace_string,
//                            'class' => 'btn btn-click btn-sm' . (($replace_string == 'Active') ? ' btn-success' : ' btn-warning'),
//                            'id' => end($row_data)
//                        );                                          
//                        $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), anchor('javascript:;', form_button($STATUS_BTN)), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));  
//                    }                
//                } 
                else if ((isset($val)) && $val == "vProfileImage") 
                {
                    $IMG_URL = "";
                    if ($replace_string != "" && file_exists(PROFILE_THUMB_IMAGE_PATH . $replace_string)) 
                    {
                        $IMG_URL = PROFILE_THUMB_IMAGE_URL . $replace_string;
                    } 
                    else 
                    {
                        $IMG_URL = PROFILE_THUMB_IMAGE_URL . "no_image.png";
                    }
                ...
                else if ((isset($val)) && $val == "vUserProfileImage") 
                {
                    $IMG_URL = "";
                    if ($replace_string != "" && file_exists(PROFILE_THUMB_192_146_IMAGE_PATH . $replace_string)) 
                    {
                        $IMG_URL = PROFILE_THUMB_192_146_IMAGE_URL . $replace_string;
                    } 
                    else 
                    {
                        $IMG_URL = PROFILE_THUMB_192_146_IMAGE_URL . "no_image.png";
                    }
                    $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), img(array("src" => $IMG_URL, 'class' => 'img-circle',"height"=>"45px")), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
                } 
                else if ((isset($val)) && $val == "appImage") 
                {
                    $IMG_URL = "";
                    if ($replace_string != "" && file_exists(APP_IMAGE_THUMB_188_190_PATH . $replace_string)) 
                    {
                        $IMG_URL = APP_IMAGE_THUMB_188_190_URL . $replace_string;
                    } 
                    else 
                    {
                        $IMG_URL = APP_IMAGE_THUMB_188_190_URL . "AndroidApp.png";
                    }
                    $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), img(array("src" => $IMG_URL, 'class' => 'img-circle',"height"=>"45px")), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
                }
                else if ((isset($val)) && $val == "vAvatar") 
                {
                    $IMG_URL = "";                 
                    if ($replace_string != "" && file_exists(STORE_IMAGE_THUMB_PATH . $replace_string)) 
                    {
                        $IMG_URL = STORE_IMAGE_THUMB_URL . $replace_string;
                    } 
                    else 
                    {
                        $IMG_URL = STORE_IMAGE_THUMB_URL . "AndroidApp.png";
                    }
                    $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), img(array("src" => $IMG_URL, 'class' => 'img-circle',"height"=>"45px")), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
                }
                else if ((isset($val)) && $val == "vIcon") 
                {
                    $IMG_URL = "";
                    if ($replace_string != "" && file_exists(CATEGORY_THUMB_PATH . $replace_string)) 
                    {
                        $IMG_URL = CATEGORY_THUMB_URL . $replace_string;
                    } 
                    else 
                    {
                        $IMG_URL = CATEGORY_THUMB_URL . "Android_Market.png";
                    }
                    $custom_val['content'] = str_replace(str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']), img(array("src" => $IMG_URL, 'class' => 'img-circle',"height"=>"45px")), str_ireplace('$' . ($key + 1), $replace_string, $custom_val['content']));
                }
                else 
