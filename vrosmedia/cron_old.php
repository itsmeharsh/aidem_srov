  ...
  $current_date=date('m/d/Y h:i:s a', time());
  		...
  		foreach ($event_exist_app_event as  $evalue){
            ...
            if($event_start_date < $current_date)
               {
               	$that_data['deleted'] ='1';
                $that_sql_query = 'UPDATE `app_event` SET deleted = "' . $that_data['deleted'] . '" WHERE id = "' . $evalue['id'] . '"';
...
//if($event_search_data['error_description']!=""){
//    die($event_search_data['error_description']);
//}
...
for($page = 1; $page <= $event_search_data['pagination']['page_size']; $page++){
   ...
   // print_r($event_list);
    ...
    foreach ($event_list as $ekey => $evalue){
        ...
        $event_exist = "SELECT id FROM `app_event` WHERE eventfull_id = '".$evalue['id']."'";
